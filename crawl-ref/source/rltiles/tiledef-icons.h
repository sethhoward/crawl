// This file has been automatically generated.

#pragma once

#include "tiledef_defines.h"

#include "tiledef-gui.h"


enum tile_icons_type
{
    TILEI_TRAP_NET = TILEG_GUI_MAX,
    TILEI_MASK_DEEP_WATER,
    TILEI_MASK_SHALLOW_WATER,
    TILEI_MASK_DEEP_WATER_MURKY,
    TILEI_MASK_SHALLOW_WATER_MURKY,
    TILEI_MASK_DEEP_WATER_SHOALS,
    TILEI_MASK_SHALLOW_WATER_SHOALS,
    TILEI_MASK_LAVA,
    TILEI_CURSOR,
    TILEI_CURSOR2,
    TILEI_CURSOR3,
    TILEI_TUTORIAL_CURSOR,
    TILEI_DISABLED,
    TILEI_RAMPAGE,
    TILEI_TRAP_WEB,
    TILEI_FRIENDLY,
    TILEI_GOOD_NEUTRAL,
    TILEI_NEUTRAL,
    TILEI_ANIMATED_WEAPON,
    TILEI_SUMMONED,
    TILEI_UNREWARDING,
    TILEI_MINION,
    TILEI_POISON,
    TILEI_MORE_POISON,
    TILEI_MAX_POISON,
    TILEI_STICKY_FLAME,
    TILEI_INNER_FLAME,
    TILEI_CONSTRICTED,
    TILEI_BERSERK,
    TILEI_FRENZIED,
    TILEI_SLOWED,
    TILEI_HASTED,
    TILEI_MIGHT,
    TILEI_PETRIFYING,
    TILEI_PETRIFIED,
    TILEI_BLIND,
    TILEI_FLEEING,
    TILEI_UNAWARE,
    TILEI_CONFUSED,
    TILEI_STAB_BRAND,
    TILEI_PAIN_MIRROR,
    TILEI_RECALL,
    TILEI_DRAIN,
    TILEI_IDEALISED,
    TILEI_BOUND_SOUL,
    TILEI_INFESTED,
    TILEI_CORRODED,
    TILEI_SWIFT,
    TILEI_VILE_CLUTCH,
    TILEI_POSSESSABLE,
    TILEI_SLOWLY_DYING,
    TILEI_FIRE_CHAMP,
    TILEI_ANGUISH,
    TILEI_ANTIMAGIC,
    TILEI_CONC_VENOM,
    TILEI_DAZED,
    TILEI_FIRE_VULN,
    TILEI_INJURY_BOND,
    TILEI_PARTIALLY_CHARGED,
    TILEI_FULLY_CHARGED,
    TILEI_DEFLECT_MISSILES,
    TILEI_GHOSTLY,
    TILEI_STILL_WINDS,
    TILEI_WATERLOGGED,
    TILEI_WEAKENED,
    TILEI_TELEPORTING,
    TILEI_BRILLIANCE,
    TILEI_RESISTANCE,
    TILEI_MALMUTATED,
    TILEI_GLOW_LIGHT,
    TILEI_GLOW_HEAVY,
    TILEI_PAIN_BOND,
    TILEI_PARALYSED,
    TILEI_BULLSEYE,
    TILEI_VITRIFIED,
    TILEI_CURSE_OF_AGONY,
    TILEI_REGENERATION,
    TILEI_RETREAT,
    TILEI_VENGEANCE_TARGET,
    TILEI_TOUCH_OF_BEOGH,
    TILEI_RIMEBLIGHT,
    TILEI_UNDYING_ARMS,
    TILEI_SHADOWLESS,
    TILEI_BIND,
    TILEI_WEAK_WILLED,
    TILEI_SIGN_OF_RUIN,
    TILEI_DOUBLED_VIGOUR,
    TILEI_KINETIC_GRAPNEL,
    TILEI_TEMPERED,
    TILEI_HEART,
    TILEI_UNSTABLE,
    TILEI_LACED_WITH_CHAOS,
    TILEI_VEXED,
    TILEI_VAMPIRE_THRALL,
    TILEI_ENKINDLED_1,
    TILEI_ENKINDLED_2,
    TILEI_NOBODY_MEMORY_1,
    TILEI_NOBODY_MEMORY_2,
    TILEI_NOBODY_MEMORY_3,
    TILEI_PYRRHIC,
    TILEI_MAGNETISED,
    TILEI_FIGMENT,
    TILEI_PARADOX,
    TILEI_WARDING,
    TILEI_DIMMED,
    TILEI_SENTINEL_MARK,
    TILEI_STRONG_WILLED,
    TILEI_TESSERACT_SPAWN,
    TILEI_SUNDERING,
    TILEI_SOMETHING_UNDER,
    TILEI_NEW_STAIR,
    TILEI_NEW_TRANSPORTER,
    TILEI_ELLIPSIS,
    TILEI_MESH,
    TILEI_OOR_MESH,
    TILEI_MAGIC_MAP_MESH,
    TILEI_TRAVEL_EXCLUSION_FG,
    TILEI_TRAVEL_EXCLUSION_CENTRE_FG,
    TILEI_NUM0,
    TILEI_NUM1,
    TILEI_NUM2,
    TILEI_NUM3,
    TILEI_NUM4,
    TILEI_NUM5,
    TILEI_NUM6,
    TILEI_NUM7,
    TILEI_NUM8,
    TILEI_NUM9,
    TILEI_NUM_MINUS5,
    TILEI_NUM_MINUS4,
    TILEI_NUM_MINUS3,
    TILEI_NUM_MINUS2,
    TILEI_NUM_MINUS1,
    TILEI_NUM_ZERO,
    TILEI_NUM_PLUS1,
    TILEI_NUM_PLUS2,
    TILEI_NUM_PLUS3,
    TILEI_NUM_PLUS4,
    TILEI_NUM_PLUS5,
    TILEI_NUM_PLUS6,
    TILEI_NUM_PLUS7,
    TILEI_NUM_PLUS8,
    TILEI_NUM_PLUS9,
    TILEI_NUM_PLUS10,
    TILEI_NUM_PLUS11,
    TILEI_DEMON_NUM1,
    TILEI_DEMON_NUM2,
    TILEI_DEMON_NUM3,
    TILEI_DEMON_NUM4,
    TILEI_DEMON_NUM5,
    TILEI_ITEM_SLOT_SELECTED,
    TILEI_MDAM_LIGHTLY_DAMAGED,
    TILEI_MDAM_MODERATELY_DAMAGED,
    TILEI_MDAM_HEAVILY_DAMAGED,
    TILEI_MDAM_SEVERELY_DAMAGED,
    TILEI_MDAM_ALMOST_DEAD,
    TILEI_TRAVEL_PATH_FROM,
    TILEI_TRAVEL_PATH_FROM_1,
    TILEI_TRAVEL_PATH_FROM_2,
    TILEI_TRAVEL_PATH_FROM_3,
    TILEI_TRAVEL_PATH_FROM_4,
    TILEI_TRAVEL_PATH_FROM_5,
    TILEI_TRAVEL_PATH_FROM_6,
    TILEI_TRAVEL_PATH_FROM_7,
    TILEI_TRAVEL_PATH_TO,
    TILEI_TRAVEL_PATH_TO_1,
    TILEI_TRAVEL_PATH_TO_2,
    TILEI_TRAVEL_PATH_TO_3,
    TILEI_TRAVEL_PATH_TO_4,
    TILEI_TRAVEL_PATH_TO_5,
    TILEI_TRAVEL_PATH_TO_6,
    TILEI_TRAVEL_PATH_TO_7,
    TILEI_ICONS_MAX
};

unsigned int tile_icons_count(tileidx_t idx);
tileidx_t tile_icons_basetile(tileidx_t idx);
int tile_icons_probs(tileidx_t idx);
int tile_icons_dominoes(tileidx_t idx);
const char *tile_icons_name(tileidx_t idx);
tile_info &tile_icons_info(tileidx_t idx);
bool tile_icons_index(const char *str, tileidx_t *idx);
bool tile_icons_equal(tileidx_t tile, tileidx_t idx);
tileidx_t tile_icons_coloured(tileidx_t idx, int col);
tileidx_t tile_icons_enchanted(tileidx_t idx, int ench);
